...
def run_eda():
    ...
    choice_list = st.multiselect('컬럼을 2개 이상 선택하세요', multi_menu)
    if len(choice_list) >= 2:
        st.dataframe(df[choice_list].corr())
        fig1 = plt.figure()
        sns.heatmap(df[choice_list].corr(), vmax=1, vmin=-1, annot=True, cmap='coolwarm', linewidths=0.5, fmt='.2f')
        st.pyplot(fig1)
    else:
        pass
    ...
    fig2 = sns.pairplot(data=df, vars=['Age','Annual Salary','Credit Card Debt','Net Worth', 'Car Purchase Amount' ])
    st.pyplot(fig2)
